...
int32_t OsDeltaTime::tick() const {
    return value;
}
...
bool operator< (OsDeltaTime const& lhs, OsDeltaTime const& rhs){ return lhs.tick() < rhs.tick(); }
bool operator> (OsDeltaTime const& lhs, OsDeltaTime const& rhs){ return rhs < lhs; }
bool operator<=(OsDeltaTime const& lhs, OsDeltaTime const& rhs){ return !(lhs > rhs); }
bool operator>=(OsDeltaTime const& lhs, OsDeltaTime const& rhs){ return !(lhs < rhs); }

uint32_t OsTime::tick() const {
    return value;
}
