...
void test_incorrect_mac_command()
{
    ...
    // FPort = 0
    nextResponse = make_data_response(
        0, std::vector<uint8_t>{0x7F}, false, server_state);
    nextResponse.time = nextPacket.time + RECEIVE_DELAY2;
    dut::send_data(nextResponse);
    ...
    // FPort = 0
    nextResponse = make_data_response(
        0, std::vector<uint8_t>{0x06, 0x03, 0x01, 0x00, 0x00}, false, server_state);
    nextResponse.time = nextPacket.time + RECEIVE_DELAY2;
    dut::send_data(nextResponse);
    ...
    // FPort = 0
    // nextResponse = make_data_response(
    //    0, std::vector<uint8_t>{0x06, 0x03, 0x01, 0x00, 0x00, 0x7F}, false, server_state);
    // nextResponse.time = nextPacket.time + RECEIVE_DELAY2;
